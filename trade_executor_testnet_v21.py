    ...
    async def update_trailing_stops(self) -> None:
        ...
        while True:
            ...
            for symbol in list(self.state.active_orders.keys()):
                ...
                # If exit detected via polling - OBSERVE ONLY (WebSocket is authoritative)
                if exit_price is not None:
                    # v2.2: Polling is detection-only, does NOT advance state
                    # WebSocket on_order_update() is the sole authority for exit finalization
                    self.logger.warning(
                        f"⏳ POLL_DETECTED_FILLED_AWAITING_WS [{trade_id}] {symbol} | "
                        f"order_id={order_id} @ {exit_price} | Awaiting WebSocket confirmation"
                    )
                    
                    # Mark position as poll-detected (advisory flag only)
                    if symbol in self.state.active_orders:
                        self.state.active_orders[symbol]['poll_exit_detected'] = True
                        self.state.active_orders[symbol]['poll_exit_price'] = exit_price
                        self.state.active_orders[symbol]['poll_exit_time'] = exchange_time
                        save_state_to_file(self.state_file, self.state.active_orders)
                    
                    # Do NOT call _handle_exit(), do NOT delete from active_orders
                    # WebSocket will finalize when ORDER_TRADE_UPDATE arrives
                    continue
