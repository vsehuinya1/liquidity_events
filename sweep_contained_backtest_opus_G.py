# sweep_contained_backtest_opus_G.py
# OPUS OPTION G: TREND ALIGNED (EMA FILTER)
# Base: Fusion (Strict/Loose Dynamic)
# Filter: Trend Direction (Only Trade With Trend)
        ...
        if is_thin_enough and bars_since_cluster <= MAX_BARS_SINCE_CLUSTER:
            ...
            if is_pocket:
                ...
                if not pd.isna(ema_val):
                    ...
                    if is_valid_long and row['close'] > ema_val: 
                        active_bias = 'LONG'
                    # Only SHORT if Price < EMA
                    elif is_valid_short and row['close'] < ema_val: 
                        active_bias = 'SHORT'
                else:
                    # Fallback if no EMA yet
