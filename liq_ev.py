...
# ============================================================================
# ENHANCED LIQUIDITY-EVENT-DRIVEN TRADING SYSTEM - SEPTEMBER 2024
# ============================================================================
# Optimized for: >60 trades, >+3bp average P&L, >45% win-rate
# ============================================================================

print('='*80)
print('ENHANCED LIQUIDITY-EVENT-DRIVEN TRADING SYSTEM - SEPTEMBER 2024')
print('Data: September 1-30, 2024 (1-minute OHLCV)')
print('='*80)

# Load September 2024 data
bars = pd.read_parquet('data/parquet/SOLUSDT_1m.parquet')
print(f'Loaded {len(bars):,} 1-minute bars from {bars.index.min()} to {bars.index.max()}')

# Filter to September 2024
bars.index = bars.index.tz_convert('UTC')
start_date = '2024-09-01 00:00:00+00:00'
end_date = '2024-09-30 23:59:59+00:00'
bars = bars.loc[start_date:end_date].copy()
print(f'Filtered to test period: {bars.index.min()} to {bars.index.max()} ({len(bars):,} bars)')

if len(bars) == 0:
    print('ERROR: No data found for September 1-30, 2024')
    exit()
...
if trade_records:
    ...
    # Save results
    trade_df.to_csv('data/processed/september_trades_enhanced.csv', index=False)
    print(f"\n✓ Detailed trade log saved: data/processed/september_trades_enhanced.csv")
