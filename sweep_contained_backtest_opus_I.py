# sweep_contained_backtest_opus_I.py
# OPUS OPTION I: IMPROVED HOLISTIC (ADX > 40)
# Base: Fusion (Strict/Loose Dynamic)
# Filter: Trend Filter active only when ADX > 40 (Catastrophe Avoidance)
...
# Shared Params
TREND_EMA_PERIOD = 600         # 10 Hours (Trend Filter)
ADX_PERIOD = 14                # Regime Filter
ADX_THRESHOLD = 40             # > 40 = EXTREME Trend Only (User Request)
...
def run_backtest(args):
    ...
    _pDM = np.where((up > down) & (up > 0), up, 0.0)
    _mDM = np.where((down > up) & (down > 0), down, 0.0)
    
    # Smooth (Wilder's: alpha=1/n)
    tr_smooth = tr.ewm(alpha=1/ADX_PERIOD, adjust=False).mean()
    pDM_smooth = pd.Series(_pDM, index=df.index).ewm(alpha=1/ADX_PERIOD, adjust=False).mean()
    mDM_smooth = pd.Series(_mDM, index=df.index).ewm(alpha=1/ADX_PERIOD, adjust=False).mean()
    ...
    for i in range(50, len(df) - 2):
        ...
        if is_thin_enough and bars_since_cluster <= MAX_BARS_SINCE_CLUSTER:
            ...
            if is_pocket:
                # REGIME SUPERVISOR (Option H)
                adx = row['adx']
                ema_val = row['trend_ema']
                
                # Default: Accept signal
                final_bias = None
                if is_valid_short: final_bias = 'SHORT'
                elif is_valid_long: final_bias = 'LONG'
                
                # If TREND REGIME (ADX > 25), Enforce Filter
                if not pd.isna(adx) and adx > ADX_THRESHOLD:
                    if not pd.isna(ema_val):
                        # Only allow if aligned with EMA
                        if final_bias == 'LONG' and row['close'] > ema_val:
                            active_bias = 'LONG'
                        elif final_bias == 'SHORT' and row['close'] < ema_val:
                            active_bias = 'SHORT'
                        else:
                            active_bias = None # Blocked by Trend Filter
                    else:
                        active_bias = final_bias # No EMA yet
                else:
                    # RANGE REGIME (ADX < 25), Allow All (Option F)
                    active_bias = final_bias
